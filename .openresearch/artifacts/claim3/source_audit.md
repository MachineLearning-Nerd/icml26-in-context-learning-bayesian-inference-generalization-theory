...
`C=min_j D_j^2/[8(nu_j^2+b_j D_j/2)]`.

The exact conclusion is an upper bound on expected mixture posterior variance,
not an equality and not a fitted empirical decay rate. The first term is the
true task family's minimax prediction risk; the task-identification remainder
has coefficient `5 B_f^2`.

The reconstructed proof uses only these assumptions. The cancellation of input
likelihoods in Equation (15) relies on the common `P_X` in Definition 1.
